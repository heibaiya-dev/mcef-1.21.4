...
public class MCEFSettings {
    ...
    private final AtomicInteger deleteRetries = new AtomicInteger(0);

    private volatile boolean skipDownload;
    private volatile String downloadMirror;
    ...
    public void load() throws IOException {
        ...
        try {
            ...
            // Reset delete retries on successful load
            deleteRetries.set(0);
        } catch (Exception e) {
            // Delete and re-create the file if there was a parsing error
            if (deleteRetries.incrementAndGet() > 20) {
                MCEF.getLogger().error("Failed to parse MCEF settings after 20 retries. Keeping existing settings.");
                return; // Stop gap
            }
            file.delete();
            save();
